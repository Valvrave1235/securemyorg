...
const PricingCard = ({ title, price, features, footnote, buttonText }) => {
    ...
    // Calculate the mask image based on mouse position
    const maskImage = useTransform([mouseX, mouseY], ([x, y]) =>
        `radial-gradient(circle at ${x}px ${y}px, rgba(255,255,255,0.8) 0%, rgba(0,0,0,0) 100%)`);
    ...
    return (
        ...
        <motion.div className="rounded-xl border border-gray-500 flex flex-col justify-between h-full relative overflow-hidden" onMouseMove={handleMouseMove}>
            <motion.div
